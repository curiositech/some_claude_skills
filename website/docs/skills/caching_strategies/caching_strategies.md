...
- **[React Performance Optimizer](/docs/skills/react_performance_optimizer)**: Client-side caching (memoization, SWR) complements server-side cache architecture
- **[Performance Profiling](/docs/skills/performance_profiling)**: Profiling identifies cache miss hotspots that caching strategies then address
- **[Cloudflare Worker Dev](/docs/skills/cloudflare_worker_dev)**: Edge caching with Cloudflare KV/Cache API is a key tier in multi-level cache architectures

## References

- [HTTP Caching Reference](./references/http-caching)
- [Redis Caching Patterns Reference](./references/redis-patterns)
