...
- **[Devops Automator](/docs/skills/devops_automator)**: Docker images are built, tested, and deployed through CI/CD automation pipelines
- **[Github Actions Pipeline Builder](/docs/skills/github_actions_pipeline_builder)**: Container builds and registry pushes are common GitHub Actions workflow stages
- **[Site Reliability Engineer](/docs/skills/site_reliability_engineer)**: Container health checks, resource limits, and orchestration are SRE responsibilities
- **[Microservices Patterns](/docs/skills/microservices_patterns)**: Microservices are typically deployed as individual Docker containers

## References

- [Docker Compose Advanced Patterns](./references/compose-patterns)
- [Multi-Stage Build Patterns](./references/multi-stage-patterns)
...
Implement the health check strategy selector below. Different application types need different health check patterns — a database-backed API needs to verify the DB connection, while a worker process might check queue depth.

```
function selectHealthCheck(service) {
  // TODO(human): Implement health check strategy selection
  // Input: { type: 'api' | 'worker' | 'static' | 'database',
  //          hasDatabase: boolean,
  //          hasRedis: boolean,
  //          framework: string }
  // Output: {
  //   dockerHealthcheck: string,    // HEALTHCHECK CMD for Dockerfile
  //   composeHealthcheck: object,   // healthcheck config for compose
  //   endpoint?: string,            // /health endpoint implementation hint
  //   checks: string[]              // what the health check should verify
  // }
  //
  // Consider:
  // - Liveness vs readiness distinction
  // - Cascading failures (if DB is down, is the API "unhealthy"?)
  // - Startup grace period (--start-period)
  // - Cost of health check (don't query DB every 5 seconds)
}
```
