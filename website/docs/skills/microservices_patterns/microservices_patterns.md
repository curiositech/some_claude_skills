...
`microservices` `patterns` `service-decomposition` `saga-pattern`

## 🤝 Pairs Great With

- **[Api Architect](/docs/skills/api_architect)**: Inter-service communication design (REST, gRPC, events) is core to microservice architecture
- **[Docker Containerization](/docs/skills/docker_containerization)**: Each microservice is typically containerized and independently deployable
- **[Code Architecture](/docs/skills/code_architecture)**: Service internal architecture patterns (hexagonal, clean) apply within each microservice
- **[Background Job Orchestrator](/docs/skills/background_job_orchestrator)**: Saga patterns and event-driven workflows use background job queues for orchestration

## References

- [Inter-Service Communication Patterns](./references/communication-patterns)
- [Service Decomposition Strategies](./references/decomposition-strategies)
