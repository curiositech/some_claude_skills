...
description: Curates, documents, and maintains a library of reusable DAG templates for common problem types. Each template is a parameterized DAG with pre-assigned skills, model tiers, and human gates.
  Use when creating a new template, browsing existing templates, parameterizing a template for a specific user, or analyzing template performance. Activate on "DAG template", "template library", "reusable
  workflow", "pre-built DAG", "workflow template", "template gallery". NOT for building one-off DAGs from scratch (use task-decomposer + dag-planner), executing templates (use dag-runtime), or creating
  skills (use skill-architect).
...
metadata:
  ...
  - skill: task-decomposer
    reason: Decomposed task patterns become parameterized templates stored in the library
  - skill: reactflow-expert
    reason: DAG templates are visualized and browsed through ReactFlow node graph interfaces
  - skill: skillful-subagent-creator
    reason: Template nodes specify which subagent skills to assign for each task type
  deprecated: true
---
